...
def make_drawers(dx, dy, dz, arrangement):
    ...
    drawer_height = available_z_space / len(arrangement)
    drawer_depth = dy - BACK_INSET - THICKNESS_1
    offsets = []
    for i in range(len(arrangement) - 1):
        offsets.append(THICKNESS_0 + (i + 1) * (drawer_height + THICKNESS_0))
    drawer_box = make_drawer_box(dx, dy, dz, offsets)
    drawers = []
    for level, num_drawers in enumerate(arrangement):
        drawer_width = (dx - THICKNESS_0 * 2) / float(num_drawers)
        z_pos = dz - (level + 1) * (THICKNESS_0 + drawer_height)
        for drawer_index in range(num_drawers):
            drawer = make_drawer(drawer_width, drawer_depth, drawer_height)
            _move(drawer, THICKNESS_0 + drawer_width * drawer_index, 0, z_pos)
